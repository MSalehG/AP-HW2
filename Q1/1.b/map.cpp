...
void Map::findRoute()
{
	int D{}, row{}, column{}, r{}, c{}, d1{}, d2{}, d3{}; //D stores the distance and d1 to d3 store the differnce between the current block and the adjacent blocks
	...
	for (int i{}; i < 2 * n - 2; i++)
	{
		...
		if ((row < n - 1) && (column < n - 1))
		{
			...
			d3 = abs(obstacle[row][column] - obstacle[row + 1][column + 1]); //The distance between the current block and the block below and right
			if (d1 <= d2 && d1 <= d3)
			{
				row = row + 1;
				route[row][column] = 1;
		...
		else if (row == n - 1 && column == n - 1) //If we have reached the end break out of the loop this if executes only if we take one or more diagonal paths
			break;
